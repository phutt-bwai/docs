...
### OpenClaw — The Fastest Way In

```bash
npx clawhub@latest install zonein
```

The [ZoneIn OpenClaw skill](https://clawhub.ai/phutt-bwai/zonein) gives any MCP-compatible AI assistant full access to ZoneIn's intelligence and agent layer. Install, set your API key, and start asking:

- _"What are the top perp signals?"_ — SM consensus across all Hyperliquid perps
- _"Show me SM consensus on ETH"_ — Detailed signal breakdown for a specific asset
- _"Create a BTC momentum agent"_ — Deploy a trading agent from natural language
- _"Backtest my agent over 30 days"_ — Run historical simulation before going live

PM signals, leaderboards, wallet tracking, agent lifecycle, vault management, backtesting — every capability available via REST is available through natural language.

### Claude Desktop

Configure the MCP server in Claude Desktop settings. Claude discovers all available tools automatically. Ask anything about signals, traders, or agents.

Every command is safety-gated — the assistant must present a summary and get your explicit `--confirm` before executing financial operations.
